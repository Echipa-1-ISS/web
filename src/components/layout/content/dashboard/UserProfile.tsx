  ...
  return (
    <div>
      ...
      >
        ...
        <p>
          <br />
          <Paragraph editable={{ onChange: setAge.toString}}>Age: {age}</Paragraph>
        </p>
        <Paragraph editable={{ onChange: setEmail}}>Email: {email}</Paragraph>

        <Upload.Dragger multiple={false} action={'http://localhost:3000/dashboard'} listType={'picture'}>
          <Button style={{width: 200}} icon={<UploadOutlined />}>Click to Upload</Button>
        </Upload.Dragger>
      </Card>
    </div>
  );
